...
function get_the_image( $args = array() ) {
    ...
    if ( $image ) $image = display_the_image( $args, $image );
...
function image_by_custom_field( $args = array() ) {
    ...
    if ( !is_array( $args['custom_key'] ) ) $args['custom_key'] = preg_split( '#\s+#', $args['custom_key'] );
    ...
    if ( $image ) return array( 'url' => $image );
    return false;
}
...
function image_by_the_post_thumbnail( $args = array() ) {
    ...
    if ( empty( $post_thumbnail_id ) ) return false;
...
function display_the_image( $args = array(), $image = false ) {
	...
	$html = '<img src="' . $imageurl . '" alt="' . esc_attr( strip_tags( get_post_field( 'post_title', $post_id ) ) ) . '" class="' . $class . '" width="' . $width . '" height="'. $height . '" />';
